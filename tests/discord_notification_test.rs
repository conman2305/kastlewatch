...
async fn test_discord_notification_flow() -> anyhow::Result<()> {
    ...
    let secret = Secret {
        ...
        string_data: Some(BTreeMap::from([("url".to_string(), mock_server.uri())])),
        ..Default::default()
    };
    ...
    let monitor = TCPMonitor::new(
        "test-monitor-notify",
        kastlewatch::shared::resources::monitors::tcp_monitor::v1alpha1::TCPMonitorSpec {
    ...
    assert_eq!(
        color, 65280,
        "Color should be Green (65280) for Healthy state"
    );
